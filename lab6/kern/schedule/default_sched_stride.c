...
proc_stride_comp_f(void *a, void *b)
{
     ...
     else if (p->lab6_stride < q->lab6_stride) {
          return -1;
     }
     else {
          return 0;
...
stride_pick_next(struct run_queue *rq)
{
     ...
     list_entry_t *min_le = le;
     while ((le = list_next(le)) != &(rq->run_list)) {
          struct proc_struct *cur = le2proc(le, run_link);
          struct proc_struct *minp = le2proc(min_le, run_link);
          if (cur->lab6_stride < minp->lab6_stride) {
               min_le = le;
          }
     }
     struct proc_struct *p = le2proc(min_le, run_link);
#endif
