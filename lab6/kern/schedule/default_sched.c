...
{
    // LAB6: 2313123
    // 断言：确保进程的run_link链表节点是空的（不在其他队列中）
    assert(list_empty(&(proc->run_link)));
    
    // 将进程插入到运行队列的尾部（循环链表，在run_list前插入就是尾部）
    list_add_before(&(rq->run_list), &(proc->run_link));
    
    // 如果进程的时间片为0或大于最大时间片，则重置为最大时间片
...
{
    // LAB6: 2313123
    // 断言：确保进程的run_link不为空且确实属于这个运行队列
...
RR_pick_next(struct run_queue *rq)
{
    ...
    // 如果le指向队列头（run_list），说明队列为空
    if (le != &(rq->run_list)) {
        // 使用le2proc宏将链表节点转换为进程结构体指针
