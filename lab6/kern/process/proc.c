...
static void sync_process_count(void);
...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
         ...
         */
        proc->wait_state = 0;      // 初始等待状态为0（不等待）
        proc->cptr = NULL;         // 没有子进程
        proc->yptr = NULL;         // 没有下一个兄弟进程
        proc->optr = NULL;         // 没有上一个兄弟进程

        
        // LAB6:2313123 (update LAB5 steps)
        /*
         ...
         *       uint64_t lab6_stride;                       // stride value (lab6 stride)
...
{
    // 安全检查
    if (proc->list_link.prev != NULL || proc->list_link.next != NULL) {
        // 如果链表节点不干净，先初始化
...
{
    // 从进程链表删除
    // 使用 list_del_init 确保节点被重置
    list_del_init(&(proc->list_link));
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    local_intr_save(intr_flag);
    {
        ...
        // LAB5 修改：使用 set_links 而不是直接 list_add
        set_links(proc);  // LAB5 的方式：设置完整的进程关系链接
...
static void sync_process_count(void) {
    ...
    // 修复：idleproc 不在 proc_list 中，但应该被计入 nr_process
    // 实际进程总数 = proc_list中的数量 + 1 (idleproc)
    int expected_nr_process = count + 1;
    
    if (expected_nr_process != nr_process) {
        nr_process = expected_nr_process;
    }
}
...
void proc_init(void)
{
    ...
    // 修复：idleproc 需要被计入 nr_process
    // 但不加入 proc_list（因为它是特殊进程）
    nr_process++;
