...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2313686 : (update LAB6 steps)
        /*
         * below fields(add in LAB8) in proc_struct need to be initialized
...
void proc_run(struct proc_struct *proc)
{
    ...
    //LAB8 2313686 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2313686  HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
    ...
    // 5. setup trapframe/context
    copy_thread(proc, stack, tf);

    // 6. link into process lists
    bool intr_flag;
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
...
{
    /* LAB8:EXERCISE2 2313686  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    //(5) set current process's mm, cr3, and set satp reg = physical addr of Page Directory
    mm_count_inc(mm);
    current->mm = mm;
    current->pgdir = PADDR(mm->pgdir);
    lsatp(PADDR(mm->pgdir));

    //(6) setup user stack with argc/argv
    ...
    stacktop -= argc * sizeof(uintptr_t);
    ...
    uintptr_t uargv_start = stacktop;
